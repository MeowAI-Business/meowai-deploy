...
use cliclack::{confirm, log, note, spinner};
...
async fn run_onboard(args: &OnboardArgs) -> Result<()> {
    ...
    note(
        ...
        format!(
            ...
            token_sync.reused,
            token_sync.updated,
            if status_key.created { "已创建" } else { "已复用" },
    ...
    log::success("下游基础服务、管理员、价格、渠道和 public status 初始化完成")
        .map_err(AppError::from_prompt)?;
    Ok(())
}

async fn run_sync(args: &SyncArgs) -> Result<()> {
    let mut config = load_deployment_config(&args.deployment.directory)?;
    config.resolve_passwords();
...
async fn run_status(args: &DeploymentArgs) -> Result<()> {
    let config = load_deployment_config(&args.directory)?;
    ...
    note(
        ...
        format!(
            ...
            if deployment.state.last_sync_success { "成功" } else { "尚未成功" },
            phases,
            compose_status
        ),
    )
...
async fn run_logout(args: &DeploymentArgs) -> Result<()> {
    let config = load_deployment_config(&args.directory)?;
    let executor = TargetExecutor::new(config.target.clone(), config.directory.clone());
    executor.run_in_directory("rm -f session.json")?;
    log::success("已删除本地短期会话文件；部署密钥和源站 Token 未撤销")
        .map_err(AppError::from_prompt)?;
    Ok(())
}
...
async fn run_rollback(args: &RollbackArgs) -> Result<()> {
    ...
    deployment.executor.run_in_directory(
        "rm -f deployment.toml state.json secrets.env docker-compose.yml kuma-helper.js\nrm -rf data",
    )?;
    log::success("下游 Compose 项目、配置和数据已清理").map_err(AppError::from_prompt)?;
    Ok(())
}

fn load_deployment_config(directory: &Path) -> Result<DeploymentConfig> {
    let path = directory.join("deployment.toml");
    let mut config = DeploymentConfig::from_file(&path)?;
    config.normalize();
    config.validate()?;
    Ok(config)
